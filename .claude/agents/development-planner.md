...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 작업은 정상 시나리오·에러 처리·엣지 케이스를 모두 포괄하는 꼼꼼한 Playwright MCP 테스트 계획을 함께 작성**
- **구현 완료 → 테스트 수행은 선택이 아닌 필수 절차이며, 테스트를 통과하기 전까지 해당 구현 사항을 완료(✅)로 표시하지 않는다**
- 각 구현 단계 완료 후 테스트 수행 및 결과 검증 (테스트 미수행/실패 항목은 미완료 상태로 유지)
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **구현 직후, 예외 없이 Playwright MCP로 테스트를 수행한다 (API 연동·비즈니스 로직 작업은 특히 정상/에러/엣지 케이스를 빠짐없이 검증)**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행 (`browser_navigate`, `browser_click`, `browser_snapshot`, `browser_network_requests` 등으로 실제 동작 확인 — 코드 리딩이나 추정으로 대체 금지)
- **테스트를 통과하지 못하면 다음 단계로 진행하지 않고 원인을 수정한 뒤 재테스트한다 (구현 → 테스트 → 통과는 하나의 게이트로 취급)**
...
- **Task 007: 부가 기능 및 사용자 경험 향상**
  - 고급 사용자 기능 구현
  - 실시간 기능 [WebSocket, SSE(Server-Sent Events) 등]
  - 파일 업로드 및 미디어 처리
...
#### **테스트 체크리스트 작성 규칙 (API 연동 및 비즈니스 로직 Task 필수)**

API 연동 또는 비즈니스 로직을 포함하는 모든 Task는 아래 규칙에 따라 "## 테스트 체크리스트" 섹션을 반드시 포함한다.

1. **테스트 범주 3종을 모두 포괄**
   - **정상 시나리오**: 기대되는 입력·조건에서 의도대로 동작하는지 검증
   - **에러 처리**: 잘못된 입력, API 실패/타임아웃, 네트워크 오류, 인증 실패 등에 대한 처리 검증
   - **엣지 케이스**: 빈 데이터, 경계값, 대량 데이터, 중복 요청 등 예외적 상황 검증
2. **Playwright MCP로만 검증**: `browser_navigate`, `browser_click`, `browser_fill_form`, `browser_snapshot`, `browser_network_requests` 등을 사용해 실제 브라우저 동작으로 확인하며, 코드 리딩이나 추정으로 대체하지 않는다
3. **게이트 원칙**: 구현이 끝났어도 테스트를 수행하고 결과(통과/실패)를 작업 파일에 기록하기 전까지는 해당 Task를 완료(✅)로 표시하지 않는다. 테스트 실패 시 원인을 수정하고 재테스트한 뒤에만 완료 처리한다
4. **결과 기록**: 테스트 항목마다 실행 여부와 통과/실패를 체크박스로 남겨, 로드맵 업데이트 시 어떤 시나리오까지 검증됐는지 추적 가능하게 한다
...
- [ ] 테스트 체크리스트가 정상 시나리오·에러 처리·엣지 케이스 3범주를 모두 포괄하는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트가 고려되었는가?
- [ ] "구현 완료 → 테스트 통과" 게이트가 지켜져, 테스트 미수행/실패 항목이 완료(✅)로 잘못 표시되지 않았는가?
